      ...
      function RecentUserDiscussionThread()
    {
        ...
        $num_rows = mysql_num_rows($res);
         if ($num_rows)
         {
             ...
             while($row = mysql_fetch_array($res))
             {
                    ...
                    $RecentChat .=   substr($row['chat'],0,50) . "...<i><font color='#6297BC'> - " . $row['chatter']. "</font></i><i><font color='#C0C0C0'>  " . "</font></i><br>";
                 ...
                 if ($num_rows)
                 {
                     ...
                     {
                         $RecentChat .=  "<font color='green'> &#8801;&#8801; Re: " . $row['chatter']. "</font><i> - <font color='#707070'> " . substr($row['chat'],0,50) . "</font>  " . "</i><br>";
                     }
                 }
                 else
                 {
                     $RecentChat .=  "<font color='#C0C0C0'> &#934; - Be the first to reply on discussion.</font><br>";
                 }
     ...
     function DiscussionThreadForOthers()
     {
        ...
        $num_rows = mysql_num_rows($res);
         ...
         {
             $heading = false;
             $RecentChat  = "<b><u>Recent Discussion</u></b>";
             while($row = mysql_fetch_array($res))
             {
               
                 $RecentChat .=  "<br><b>&#8801;</b> ". substr($row['chat'],0,50) . "...<i><font color='#6297BC'>  " . $row['chatter']. "</font></i><i><font color='#C0C0C0'> " . "</font></i><br>";
                 ...
                 if ($num_rows)
                 {
                     while($row = mysql_fetch_array($res1))
                     {
                         $RecentChat .=  "<font color='green'> &#8801;&#8801;Re: " . $row['chatter']. "</font><i> - " . substr($row['chat'],0,50) . "</i><br>";
                     }
                 }
                 else
                 {
     ...
     function FlashMessage()
     {
         ...
         $HowToUseDiscussion = "<font color='green'>Please do good <i>rating & review</i> for other users</font><font color='blue'> Click here...</font>";
         
         $FlashMessage = $HowToUseDiscussion .  "<br>"  . RecentUserDiscussionThread() . "<br>"  . DiscussionThreadForOthers() . "<br>" .  RecentChatDiscussion(" where _to = '-999' or _to = ''", "6") . "<br>"  . Online_User_List() ;
         return $FlashMessage;
     }
     ...
     function DiscussionStatus()
     {
         if (Chat_tbl_app_chat_Discussion_Posted_Counter() == "0")
            ...
            /************** Private Msg Status   **************/
     function PrivateMsgStatus()
     {
        
          $q =  "select count(*) n from tbl_app_msg where session_id_to = '".$_REQUEST['session_id']."' and iRead = 0;";
